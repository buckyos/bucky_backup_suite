...
class CreatePlanDlg extends LitElement {
    ...
    async readDataFromUI() : Promise<boolean> {
        const plan_combo = this.shadowRoot?.querySelector("#backup-plan-combo") as SlSelect;
        if (plan_combo) {
            if (plan_combo.selectedOptions.length < 1) {
                alert("You should select a backup plan");
                return false;
            }
           this.ownerWizzard.wizzard_data.plan_id = plan_combo.selectedOptions[0].value;
        }
        const checkpoint_combo = this.shadowRoot?.querySelector("#checkpoint-combo") as SlSelect;
        if (checkpoint_combo) {
            if (checkpoint_combo.selectedOptions.length < 1) {
                alert("You should select a checkpoint");
                return false;
            }
           this.ownerWizzard.wizzard_data.checkpoint_id = checkpoint_combo.selectedOptions[0].value;
        }
        return true;
    }

    firstUpdated() {
        const backupPlanCombo = this.shadowRoot?.querySelector("#backup-plan-combo") as SlSelect;
        const checkpointContainer = this.shadowRoot?.querySelector("#checkpoint-container") as HTMLElement;
        const checkpointCombo = this.shadowRoot?.querySelector("#checkpoint-combo") as SlSelect;
        ...
        if (nextButton) {
            nextButton.addEventListener("click", async () => {
                if (await this.readDataFromUI()) {
                    let select_target_dlg = document.createElement("restore-select-target-dlg") as RestoreSelectTargetDlg;
                    if (this.ownerWizzard) {
                        select_target_dlg.setOwnerWizzard(this.ownerWizzard);
                        this.ownerWizzard.pushDlg(select_target_dlg,"Where to restore?");
                    }
                }
            });
        }
        ...
        let load_plan_list = async () => {
            try {
                let plan_ids = await taskManager.listBackupPlans() as string[];
                let plans = await Promise.all(plan_ids.map(plan_id => taskManager.getBackupPlan(plan_id)));
                plans.forEach(plan => {
