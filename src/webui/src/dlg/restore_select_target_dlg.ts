...
export class RestoreSelectTargetDlg extends LitElement {
    ...
    async readDataFromUI() {
        let restore_target_path = this.shadowRoot?.querySelector(
            "#restore-target-path"
        ) as SlInput;
        ...
        try {
            const path_exist = await taskManager.validatePath(
                restore_target_path.value
            );
            if (!path_exist) {
                alert(
                    "Invalid path, please confirm the path exists and has access permission"
                );
                return false;
            }
        } catch (error) {
        ...
        let is_clean_folder = this.shadowRoot?.querySelector(
            "#is-clean-folder"
        ) as SlCheckbox;
        if (is_clean_folder) {
            this.ownerWizzard!.wizzard_data.is_clean_folder =
                is_clean_folder.checked;
        }
