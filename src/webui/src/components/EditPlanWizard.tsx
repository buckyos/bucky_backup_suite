...
export function EditPlanWizard({ onBack, onComplete, data }: EditPlanWizardProps) {
  const { t } = useLanguage();
  ...
  const [planDescription, setPlanDescription] = useState(data?.description || '每日自动备份系统关键文件');
        ...
        return (
          <div className="space-y-6">
            <div>
              ...
              <p className="text-sm text-muted-foreground mb-4">修改备份计划的基本信息</p>
            </div>
            ...
            <div className="p-4 bg-amber-50 rounded-lg dark:bg-amber-950">
              <p className="text-sm text-amber-800 dark:text-amber-200">
                注意: 备份源和目标服务不能在编辑时修改。如需更改，请创建新的备份计划。
              </p>
            </div>
          </div>
        );
        ...
        return (
          <div className="space-y-6">
            <div>
              ...
              <p className="text-sm text-muted-foreground mb-4">配置备份计划的执行时间</p>
            </div>
              ...
              <div className="space-y-3">
                ...
                <RadioGroup value={triggerType} onValueChange={setTriggerType}>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <Label htmlFor="manual">仅手动执行</Label>
                  </div>
                </RadioGroup>
              </div>
        ...
        return (
          <div className="space-y-6">
            <div>
              ...
              <p className="text-sm text-muted-foreground mb-6">请确认以下修改内容</p>
            </div>
            
            <div className="space-y-4">
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">计划信息</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    <span className="text-sm text-muted-foreground">计划描述:</span>
                    <span className="text-sm font-medium">{planDescription || '无描述'}</span>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">调度设置</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div className="flex justify-between">
                    <span className="text-sm text-muted-foreground">触发方式:</span>
                    <span className="text-sm font-medium">
                      {triggerType === 'scheduled' ? '定时执行' : '仅手动执行'}
                    </span>
                  </div>
                  {triggerType === 'scheduled' && (
                    <>
                      <div className="flex justify-between">
                        <span className="text-sm text-muted-foreground">调度周期:</span>
                        <span className="text-sm font-medium">
                          {schedulePeriod === 'daily' ? '每天' : 
                           schedulePeriod === 'weekly' ? '每周' : '每月'}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm text-muted-foreground">执行时间:</span>
                        <span className="text-sm font-medium">{scheduleTime}</span>
                      </div>
                      {schedulePeriod === 'weekly' && (
                        <div className="flex justify-between">
                          <span className="text-sm text-muted-foreground">执行日期:</span>
                          <span className="text-sm font-medium">
                            {weekDays.map(day => {
                              const dayLabels = { '0': '周日', '1': '周一', '2': '周二', '3': '周三', '4': '周四', '5': '周五', '6': '周六' };
                      ...
                      {schedulePeriod === 'monthly' && (
                        <div className="flex justify-between">
                          <span className="text-sm text-muted-foreground">执行日期:</span>
                          <span className="text-sm font-medium">每月 {monthDay} 日</span>
                        </div>
                      )}
                    </>
                  )}
    ...
    <div className={`${isMobile ? 'p-4' : 'p-6'} space-y-6`}>
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">修改备份计划的配置</p>
        </div>
      </div>
