    ...
    <div className={`${isMobile ? 'p-4 pt-16' : 'p-6'} space-y-6`}>
        ...
        <div>
          {!isMobile && (
            <>
              ...
              <p className="text-muted-foreground">配置和管理备份目标位置</p>
            </>
          )}
        </div>
        <Button 
      ...
      <div className="grid gap-4">
        {services.map((service) => {
          ...
          const usagePercent = getUsagePercentage(service.used, service.total);
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <CardDescription>
                        {service.type === 'local' ? service.path : service.endpoint}
                      </CardDescription>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
              ...
              <CardContent>
                <div className={`grid ${isMobile ? 'grid-cols-1' : 'grid-cols-1 lg:grid-cols-4'} gap-4 mb-4`}>
                  <div>
                    ...
                    <p className="font-medium">{service.used} / {service.total}</p>
                    {service.total !== '无限制' && (
                      <div className="mt-2">
                        ...
                        <p className="text-xs text-muted-foreground mt-1">{usagePercent}% 已使用</p>
                      </div>
                    )}
                  </div>
                  <div>
                    ...
                    <p className="font-medium">
                      {service.type === 'local' ? '本地目录' : 'NDN网络'}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground mb-1">压缩</p>
                    <p className="font-medium">{service.compression ? '已启用' : '已禁用'}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground mb-1">加密</p>
                    <p className="font-medium">{service.encryption ? '已启用' : '已禁用'}</p>
                  </div>
                </div>
                  ...
                  {isMobile ? (
                    <>
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            <AlertDialogTitle>删除备份服务</AlertDialogTitle>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base">总服务数</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base">在线服务</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-2xl">{services.filter(s => s.status !== 'offline').length}</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-3">
            ...
            <CardContent>
              <p className="text-2xl">{services.filter(s => s.type === 'local').length}</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base">网络服务</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-2xl">{services.filter(s => s.type === 'ndn').length}</p>
            </CardContent>
          </Card>
        </div>
      )}
