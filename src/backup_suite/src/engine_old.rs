...
impl BackupEngine {
    ...
    pub async fn create_backup_task(
        ...
        let new_checkpoint =
            BackupCheckPoint::new(plan_id, parent_checkpoint_id, last_checkpoint_index);
        ...
        all_checkpoints.insert(
            new_checkpoint.checkpoint_id.clone(),
            Arc::new(Mutex::new(new_checkpoint)),
        );
    ...
    async fn run_chunk2chunk_backup_task(
        ...
        let source3 = self
            ...
            .await?;
        let target2 = self
            .get_chunk_target_provider(target.get_target_url().as_str())
            .await?;
        ...
        let eval_thread = tokio::spawn(async move {
            ...
            let eval_result = BackupEngine::backup_chunk_source_eval_thread(
                engine_eval,
                source2,
                target,
                backup_task_eval,
        ...
        let transfer_thread = tokio::spawn(async move {
            ...
            let transfer_result = BackupEngine::backup_work_thread(
                engine_transfer,
                source3,
                target2,
                backup_task_trans,
        ...
        let is_all_done = self
            .task_db
            .check_is_checkpoint_items_all_done(&checkpoint_id)?;
        if is_all_done {
    ...
    pub async fn backup_chunk_source_prepare_thread(
        ...
        loop {
            ...
            for mut item in this_item_list.into_iter() {
                ...
                item_count += 1;
                if item.chunk_id.is_some()
                    && (item.size > SMALL_CHUNK_SIZE || !have_depend_checkpoint)
                {
                ...
                engine
                    .task_db
                    .save_backup_item(checkpoint_id.as_str(), &item)?;
                if item.have_cache {
        ...
        info!(
            "{} source.prepare_items return done, all items are prepared",
            checkpoint_id.as_str()
        );
    ...
    async fn cacl_item_hash_and_diff(
        ...
        loop {
            debug!(
                "calc full hash for item: {}, offset: {},len: {}",
            ...
            loop {
                if total_size.load(Ordering::Relaxed) < max_cache_size {
                    ...
                    debug!(
                        "add piece to cache, size: {},total_cache_size: {} MB",
                        content_len,
                        total_size.load(Ordering::Relaxed) / 1024 / 1024
                    );
                    break;
                } else {
                    //sleep
        ...
        info!(
            "calc full hash for item: {}, full_id: {}",
            backup_item.item_id,
            full_id.to_string()
        );
        Ok(full_id)
    ...
    pub async fn backup_chunk_source_eval_thread(
        ...
        loop {
            ...
            if real_checkpoint.state == CheckPointState::Evaluated {
                info!(
                    "checkpoint {} is evaluated, exit eval thread",
            ...
            loop {
                ...
                if next_item.is_some() {
                    ...
                    if backup_item.chunk_id.is_some() {
                        item_chunk_id =
                            Some(ChunkId::new(backup_item.chunk_id.as_ref().unwrap()).unwrap());
                        ...
                        if item_reader.is_err() {
                            ...
                            match err {
                                BuckyBackupError::TryLater(msg) => {
                                    warn!(
                                        "open item {} reader error: {}, try later",
                                        backup_item.item_id, msg
                                    );
                                    continue;
                                }
                                _ => {
                    ...
                    if item_chunk_id.is_some() {
                        ...
                        if is_exist {
                            ...
                            if backup_item.quick_hash.is_some() {
                                let full_chunk_id =
                                    target.query_link_target(&real_chunk_id).await?;
                                if full_chunk_id.is_some() {
                                    let full_chunk_id = full_chunk_id.unwrap();
                                    debug!(
                                        "query link target for chunk {} success, full_chunk_id: {}",
                                        real_chunk_id.to_string(),
                                        full_chunk_id.to_string()
                                    );
                                    backup_item.chunk_id = Some(full_chunk_id.to_string());
                                    engine
                                        .task_db
                                        .update_backup_item(checkpoint_id.as_str(), &backup_item)?;
                                } else {
                                    warn!(
                                        "query link target for chunk {} error",
                                        real_chunk_id.to_string()
                                    );
                                    is_item_done = false;
                                }
                            ...
                            if is_item_done {
                                info!(
                                    "item {} 's chunk_id: {}, is exist! will skip",
                                    backup_item.item_id,
                                    real_chunk_id.to_string()
                                );
                                engine
            ...
            if real_checkpoint.state == CheckPointState::Prepared {
                info!(
                    "checkpoint {} is prepared, try load new backup items from db...",
                    real_checkpoint.checkpoint_id
                );
                drop(real_checkpoint);
                let new_item_list = engine.task_db.load_wait_cacl_backup_items(&checkpoint_id)?;
                debug!(
                    "eval thread load new backup items done, item count: {}",
                    new_item_list.len()
                );
                if !new_item_list.is_empty() {
                    info!(
                        "{} new backup items are loaded to eval",
                        new_item_list.len()
                    );
    ...
    pub async fn backup_work_thread(
        ...
        loop {
            ...
            if real_checkpoint.state == CheckPointState::Done {
                info!(
                    "checkpoint {} is done, exit transfer thread",
                    real_checkpoint.checkpoint_id
                );
                drop(real_checkpoint);
                break;
            ...
            if real_checkpoint.state == CheckPointState::Evaluated {
                info!(
                    "checkpoint {} is evaluated, try load new backup items from db...",
                    real_checkpoint.checkpoint_id
                );
                let real_checkpoint_id = real_checkpoint.checkpoint_id.clone();
                drop(real_checkpoint);
                let new_item_list = engine
                    .task_db
                    .load_wait_transfer_backup_items(&real_checkpoint_id)?;

                if !new_item_list.is_empty() {
                    info!(
                        "{} new backup items are loaded to transfer",
                        new_item_list.len()
                    );
                    for item in new_item_list {
                        transfer_queue.push(item);
            ...
            loop {
                ...
                if next_item.is_some() {
                    ...
                    let open_result = target
                        .open_chunk_writer(&chunk_id, 0, backup_item.size)
                        .await;
                    if open_result.is_err() {
                        ...
                        match err {
                            BuckyBackupError::AlreadyDone(msg) => {
                                ...
                                let mut cache_mgr = CHUNK_TASK_CACHE_MGR.lock().await;
                                cache_mgr
                                    .free_chunk_cache(backup_item.chunk_id.as_ref().unwrap())
                                    .await;
                                drop(cache_mgr);
                                continue;
                            }
                            BuckyBackupError::TryLater(msg) => {
                                warn!(
                                    "open chunk {} writer error: {}, try later",
                                    chunk_id.to_string(),
                                    msg
                                );
                                continue;
                    ...
                    info!(
                        "start upload chunk {} , offset: {}, size: {}",
                        chunk_id_str, offset, backup_item.size
                    );
                    ...
                    let chunk_cache_node =
                        cache_mgr.get_chunk_cache_node(backup_item.item_id.as_str());
                    drop(cache_mgr);
                    ...
                    if chunk_cache_node.is_some() {
                        ...
                        if free_size > 0 {
                            debug!(
                                "free cache size: {},offset: {},cache_start_pos: {}",
                                free_size, offset, chunk_cache_node.start_offset
                            );
                            mgr_total_size
                                .fetch_sub(free_size, std::sync::atomic::Ordering::Relaxed);
                        }
                    }
                    ...
                    loop {
                        ...
                        if this_item_cache_node.is_some() {
                            let chunk_cache_node =
                                this_item_cache_node.as_mut().unwrap().lock().await;
                            cache_start_offset = chunk_cache_node.start_offset;
                            cache_end_offset = chunk_cache_node.end_offset;
                            debug!(
                                "cache node start offset: {}, end offset: {}",
                                cache_start_offset, cache_end_offset
                            );
                        }
                        ...
                        if offset < cache_start_offset || offset >= cache_end_offset {
                            if real_reader.is_none() {
                                debug!(
                                    "open item {} reader, offset: {}",
                                    backup_item.item_id, offset
                                );
                                let mut reader = source
                                    .open_item_reader(&backup_item.item_id, offset)
                                    .await;
                                if reader.is_err() {
                            ...
                            if offset < cache_start_offset {
                                if cache_start_offset - offset > send_buf.len() as u64 {
                                    ...
                                    read_result = reader
                                        .read(
                                            &mut send_buf[..(cache_start_offset - offset) as usize],
                                        )
                                        .await;
                                }
                            } else {
                            ...
                            if cache_piece.is_some() {
                                ...
                                debug!(
                                    "pop cache piece start offset: {}, piece len: {}",
                                    piece_start_offset,
                                    cache_piece.len()
                                );
                        ...
                        if real_task.state != TaskState::Running {
                            debug!(
                                "backup task {} is not running, break upload loop",
                                real_task.taskid
                            );
                            break;
                        }
                    ...
                    let mut cache_mgr = CHUNK_TASK_CACHE_MGR.lock().await;
                    cache_mgr
                        .free_chunk_cache(backup_item.item_id.as_str())
                        .await;
                    drop(cache_mgr);
                } else {
                    //idle
    ...
    fn check_all_check_point_exist(&self, checkpoint_id: &str) -> BackupResult<bool> {
        ...
        debug!(
            ...
            checkpoint_id,
            checkpoint.depend_checkpoint_id.as_ref().unwrap()
        );
    ...
    async fn run_chunk2chunk_restore_task(
        ...
        if need_build_items {
            ...
            let backup_items = self
                .task_db
                .load_backup_items_by_checkpoint(&checkpoint_id)?;
            info!(
                "load {} backup items for checkpoint: {}",
                backup_items.len(),
                checkpoint_id
            );
            ...
            for item in backup_items {
                let restore_item = BackupChunkItem {
            ...
            let mut real_task = restore_task.lock().await;
            self.task_db
                .save_restore_item_list_to_task(&real_task.taskid, &restore_item_list)?;
            ...
            restore_item_list = self
                .task_db
                .load_restore_items_by_task(&real_task_id, &BackupItemState::New)?;
        ...
        for item in restore_item_list {
            ...
            let open_resulut = source
                .open_writer_for_restore(&item, &restore_config, offset)
                .await;
            if open_resulut.is_err() {
                ...
                real_task.completed_item_count += 1;
                real_task.completed_size += item.size;
                self.task_db.update_restore_item_state(
                    &real_task_id,
                    &item.item_id,
                    BackupItemState::Done,
                )?;
            ...
            let progress_callback = {
                Some(
                    move |chunk_id: ChunkId, pos: u64, hasher: &Option<ChunkHasher>| {
    ...
    async fn get_chunk_source_provider(
        ...
        let store = LocalDirChunkProvider::new(local_path.to_string()).await?;
        Ok(Box::new(store))
    }
