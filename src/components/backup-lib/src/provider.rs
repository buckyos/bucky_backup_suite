...
use ndn_lib::{ChunkId, ChunkReadSeek, ChunkReader, ChunkWriter, ObjId, NdnProgressCallback};
...
pub trait IBackupChunkSourceProvider {
    ...
    async fn open_writer_for_restore(&self, restore_target_id: &str, item: &BackupChunkItem,restore_config:&RestoreConfig,offset:u64)->BackupResult<(ChunkWriter,u64)>;
}
...
pub type BackupChunkSourceCreateFunc = Box<dyn FnMut(String) -> Pin<Box<dyn Future<Output = BackupResult<BackupChunkSourceProvider>> + Send + 'static>> + Send>;
pub type BackupChunkTargetCreateFunc = Box<dyn FnMut(String) -> Pin<Box<dyn Future<Output = BackupResult<BackupChunkTargetProvider>> + Send + 'static>> + Send>;
