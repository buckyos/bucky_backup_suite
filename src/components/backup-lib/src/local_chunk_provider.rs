...
impl LocalChunkTargetProvider {
    pub async fn new(dir_path: String) -> Result<Self> {
        let chunk_store = NamedDataStore::new(dir_path.clone())
            .await
            .map_err(|e| anyhow::anyhow!("{}", e))?;
...
impl IBackupChunkTargetProvider for LocalChunkTargetProvider {
    ...
    async fn open_chunk_writer(
        ...
        let (mut writer, process) = self
            .chunk_store
            .open_chunk_writer(chunk_id, size, offset)
            .await
            .map_err(|e| match e {
    ...
    async fn link_chunkid(
        ...
        self.chunk_store
            .link_object(&from_obj_id, &to_obj_id)
            .await
            .map_err(|e| {
    ...
    async fn query_link_target(&self, source_chunk_id: &ChunkId) -> BackupResult<Option<ChunkId>> {
        ...
        let target_chunk_ids = self
            .chunk_store
            .query_link_refs(&obj_id)
            .await
            .map_err(|e| {
